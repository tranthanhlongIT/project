  ...
  <div class="m-2">
    ...
    <v-card>
      <v-card-title>
        <v-btn
          ...
          small
          class="my-0"
          @click.prevent="openDialog('add', {}, '40vw')"
        >
          Add
      ...
      <v-data-table
        ...
        @current-items="getCurrentObjectList"
      >
        <template v-slot:[`item.image`]="{ item }">
          <v-avatar class="my-2" v-if="item.image">
            <v-img
              :src="`http://localhost:8000/api/storage/app/images/${item.image}`"
            ></v-img>
          </v-avatar>
        ...
        <template v-slot:[`item.operations`]="{ item }">
          <v-icon small class="mr-2" @click="openDialog('det', item, '35vw')">
            mdi-eye
          </v-icon>
          <v-icon small class="mr-2" @click="openDialog('upd', item, '35vw')">
            mdi-pencil
          </v-icon>
    ...
    >
      <div
        v-if="action === 'add'"
        slot="header"
        class="ma-1 ml-2 text-subtitle-1 indigo--text"
      >
        <v-icon dense color="indigo" class="mr-1 mb-1">mdi-information</v-icon
        >Add User
      </div>
      <div
        v-if="action === 'upd'"
...
export default {
  ...
  methods: {
    ...mapActions(["commitDialog", "getUsers"]),

    openDialog(action, object) {
      this.action = action;
      this.object = Object.assign(object);
      this.commitDialog({ dialog: true });
    },
